...
Win32Window::Win32Window(int w,int h,const wchar_t* title){HINSTANCE in=GetModuleHandleW(nullptr);WNDCLASSW wc{};wc.lpfnWndProc=wndProc;wc.hInstance=in;wc.lpszClassName=L"RetroQuakeCppWindow";wc.hCursor=LoadCursor(nullptr,IDC_CROSS);RegisterClassW(&wc);POINT origin{0,0};HMONITOR monitor=MonitorFromPoint(origin,MONITOR_DEFAULTTOPRIMARY);MONITORINFO info{sizeof(info)};GetMonitorInfoW(monitor,&info);RECT bounds=info.rcMonitor;m_hwnd=CreateWindowExW(0,wc.lpszClassName,title,WS_POPUP|WS_VISIBLE,bounds.left,bounds.top,bounds.right-bounds.left,bounds.bottom-bounds.top,nullptr,nullptr,in,this);m_bmi.bmiHeader.biSize=sizeof(BITMAPINFOHEADER);m_bmi.bmiHeader.biWidth=w;m_bmi.bmiHeader.biHeight=-h;m_bmi.bmiHeader.biPlanes=1;m_bmi.bmiHeader.biBitCount=32;m_bmi.bmiHeader.biCompression=BI_RGB;ShowCursor(FALSE);}
