...
namespace retro {
struct Door {float left=0,right=0,y=0,open=0;bool opening=false,transfer=false,entry=false;};
...
struct Structure {float x1,y1,x2,y2,bottom,top;bool rail=false;};
...
public:
    ...
    bool fits(float x,float y,float feet,float height)const;
    ...
    float supportHeight(float x,float y)const;
    float ceilingHeight(float x,float y)const;
    float clearanceHeight(float x,float y)const;
    bool rayClear(Vec2 a,float az,Vec2 b,float bz,bool doors=true)const;
    ...
    int nearbyDoor(Vec2 position,Vec2 forward)const;
