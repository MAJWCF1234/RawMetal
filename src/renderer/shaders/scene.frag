...
float shaftScattering(vec3 eye,vec3 endpoint,vec4 lamp){
 ...
 vec3 ray=endpoint-eye;float rayLength=min(length(ray),16.0);
 if(rayLength<0.01)return 0.0;
 vec3 direction=normalize(ray);
 // Restrict the march to the segment crossing this lamp's horizontal footprint.
 ...
 // Beer-Lambert transmittance keeps longer paths from blowing out the scene.
...
void main(){
 ...
 if(parallaxScale>0.0&&surface.z<0.5&&surface.x<6.0){
  ...
  if(abs(determinant)>0.00001){
   ...
   for(int layer=0;layer<4;++layer){float height=textureGrad(reliefMap,sampleUV,uvDx,uvDy).r;if(float(layer)/4.0>=height)break;sampleUV-=stepUV;}
  }
 }
 ...
 if(lighting.w>0.5){
  ...
  if(gloss>0.001){
   ...
   if(abs(determinant)>0.00001){
    vec3 tangent=normalize((dx*uvDy.y-dy*uvDx.y)/determinant);
    vec3 bitangent=normalize((dy*uvDx.x-dx*uvDx.y)/determinant);
    ...
    float sharpness=surface.z>1.5?56.0:30.0;
    if(light0.w>0.0){vec3 halfVector=light0.xyz+viewTangent;halfVector*=inversesqrt(max(dot(halfVector,halfVector),0.000001));specularLight+=light0.w*pow(max(dot(n,halfVector),0.0),sharpness);}
    if(light1.w>0.0){vec3 halfVector=light1.xyz+viewTangent;halfVector*=inversesqrt(max(dot(halfVector,halfVector),0.000001));specularLight+=light1.w*pow(max(dot(n,halfVector),0.0),sharpness);}
    specularLight=min(specularLight*gloss,0.28);
   }
  }
 }
 if(surface.z>0.5&&surface.z<1.5){outColor=vec4(color.rgb*color.a*lighting.x*vec3(1,0.72,0.35),1);return;}
 // Texture files are display encoded; light and emission must combine in
 // linear space before the display transform. Keep source texels nearest-
 // filtered, so high-resolution geometry retains the authored pixel texture.
 vec3 result=toLinear(color.rgb)*lighting.x*vertexLight/(1+surface.x*0.018);
 if(surface.z<0.5&&surface.w<1.5){
  // Keep unlit recesses cool and let the authored fixture contribution warm the
  // material; this preserves the texture color instead of tinting the frame.
  float key=clamp((vertexLight-0.24)/0.75,0.0,1.0);
  result*=mix(vec3(0.82,0.88,0.97),vec3(1.04,0.98,0.90),key);
 }
 result+=specularLight*(surface.z>1.5?vec3(0.84,0.96,1.0):vec3(1.0,0.84,0.63));
 if(surface.y>0.0)result+=toLinear(textureGrad(emissionMap,sampleUV,uvDx,uvDy).rgb)*1.6*surface.y;
 if(surface.z<0.5&&surface.w<1.5&&view.atmosphere.w>0.0){
  float distance=length(worldPos-view.eyeYaw.xyz);
  if(distance<75.0){float extinction=1.0-exp(-max(distance-4.0,0.0)*view.atmosphere.w);
   result=mix(result,toLinear(view.atmosphere.rgb),min(extinction,0.58));}
 }
 // Each opaque receiver stops the view ray, so walls and enemies clip the beam.
 if(surface.z<0.5&&surface.w<1.5){
  float scatter=0.0;
  for(int light=0;light<4;++light)scatter+=shaftScattering(view.eyeYaw.xyz,worldPos,view.fogLights[light]);
