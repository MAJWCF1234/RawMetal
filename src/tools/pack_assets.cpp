...
int main(int argc,char** argv){try{
 ...
 while(std::getline(manifest,line)){std::smatch match;if(!std::regex_search(line,match,pattern))continue;
  ...
  if(path.extension()==".png"){
   int w,h,channels;auto pixels=stbi_load_from_memory(original.data(),int(original.size()),&w,&h,&channels,4);if(!pixels)throw std::runtime_error("PNG decode failed");
   size_t count=size_t(w)*h;Bytes raw(12+count*4);std::memcpy(raw.data(),"RMT1",4);std::memcpy(raw.data()+4,&w,4);std::memcpy(raw.data()+8,&h,4);std::memcpy(raw.data()+12,pixels,count*4);stbi_image_free(pixels);
