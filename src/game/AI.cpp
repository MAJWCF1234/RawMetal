...
void Game::updateEnemies(float dt){
 ...
 bool footsteps=false;for(auto&event:m_sounds)footsteps|=event.sound>=Sound::Metal1&&event.sound<=Sound::Concrete4;
 ...
 for(auto&e:m_enemies){
  ...
  float range=e.kind==Enemy::Kind::Brute?1.25f:1.05f;
  bool sameLevel=m_player.z<e.bodyTop()&&m_player.z+m_player.hullHeight()>e.bodyBottom();
  if(e.windup>0){if(e.kind!=Enemy::Kind::Brute&&e.windup>.09f){e.heading+=wrapAngle(std::atan2(to.y,to.x)-e.heading)*std::min(1.f,dt*16.f);facing={std::cos(e.heading),std::sin(e.heading)};}e.windup-=dt;if(e.windup<=0){e.strike=1;e.attackCooldown=e.kind==Enemy::Kind::Wasp?.8f:e.kind==Enemy::Kind::Huntsman?1.f:1.6f;
    if(dist<range+.1f&&dot(normalized(to),facing)>.25f&&sameLevel&&m_world.rayClear(e.pos,e.z+.6f,m_player.pos,m_player.z+.5f))receiveDamage(e.kind==Enemy::Kind::Brute?18.f:9.f,e.pos);
   }continue;
  }
  ...
  if(visible&&sameLevel&&dist<range&&e.attackCooldown<=0&&e.strike<=0){e.windup=e.kind==Enemy::Kind::Brute?.8f:.32f;enemySound(e,1,.85f);continue;}
  if(length(goal-e.pos)<.3f||(visible&&dist<range*.82f)||e.painFlash>.65f)continue;
  ...
  if(!direct&&e.repathTimer>0)destination=e.waypoint;
   ...
   float hull=e.kind==Enemy::Kind::Brute?1.85f:e.kind==Enemy::Kind::Wasp?1.6f:1.05f;
  ...
  Vec2 separation{};for(auto&other:m_enemies)if(&other!=&e&&other.alive){auto away=e.pos-other.pos;float d=length(away);if(d>.001f&&d<.85f)separation+=away*( (.85f-d)/d);}
  ...
  auto old=e.pos;
  auto move=[&](Vec2 next){float step=e.kind==Enemy::Kind::Huntsman?.65f:.215f;float ground=groundHeight(next,e.z+step),height=e.kind==Enemy::Kind::Brute?1.85f:e.kind==Enemy::Kind::Wasp?1.6f:1.05f;
